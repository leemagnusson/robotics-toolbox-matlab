...
q00 = q0 + [-1,0,1,0,0,0,0,0];%+[-.5,0,0,.1,-.5,0,0];
T0 = r.fkine(q00);
x0 = T0(1:3,4);
xf = x0+[0;-.1;-.1];
xf = x0;
...
for i = 1:size(zyx,1)
    ...
    Tfn = rt2tr(rfs(:,:,i),xf);
    n = length(t);
    tc(:,:,(i-1)*n+(1:30)) = ctraj(Tf,Tfn,length(t));
    Tf = Tfn;
end
F
t = linspace(0,tmax*size(zyx,1),tmax*30*size(zyx,1));
...
for i = 1:length(t)-1
    ...
    q_dot(i,:) = (inv(J'*J+.0001*diag([1,1,1,1,1,.00001,.00001,.00001]))*J'*x_dot(i,:)')';
