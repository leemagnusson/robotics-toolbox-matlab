...
hold on
% input joint limits: the joint limits come from the urdf parsed data. for
% the joint limits exceed -pi to pi will be wrapped between -pi to pi.
...
if ~do_plot
    ...
    for q6 = joint_limits(1,1):(sample_step):joint_limits(2,1)
        for q7 = joint_limits(1,2):sample_step:joint_limits(2,2)
            for q8 = joint_limits(1,3):0.01:joint_limits(2,3)
                ...
                % only three joints: 6 7 and 8 are swept to generate the
                % workspace for the spherical arm
    ...
    % the data file comes from the saved mat file after sweeping the
    % workspace.
    load('/data/eef_workspace2.mat');
    % put a boundary on the data point cloud
