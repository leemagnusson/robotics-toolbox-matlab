...
load('q_init_setup_prostatectomy.mat');
load('index_joints.mat');
load('coupling_matrix.mat');
load('vertex_bed.mat');
load('vertex_bed_adapter.mat');
load('point_clouds_prostatectomy_workspace.mat');
...
q(:,1) = q_init_setup(:,1);
robot_arms{1}.transformation_base_ = transformation_base(:,:,1);
robot_arms{1}.CalculateFK(q(:,1));
p_eef(:,1) = robot_arms{1}.frames_(1:3,4,index_eef);
rotation_eef(:,:,1) = robot_arms{1}.frames_(1:3,1:3,index_eef);

q(:,2) = q_init_setup(:,2);
robot_arms{2}.transformation_base_ = transformation_base(:,:,2);
robot_arms{2}.CalculateFK(q(:,2));
p_eef(:,2) = robot_arms{2}.frames_(1:3,4,index_eef);
rotation_eef(:,:,2) = robot_arms{2}.frames_(1:3,1:3,index_eef);

q(:,3) = q_init_setup(:,3);
robot_arms{3}.transformation_base_ = transformation_base(:,:,3);
robot_arms{3}.CalculateFK(q(:,3));
p_eef(:,3) = robot_arms{3}.frames_(1:3,4,index_eef);
rotation_eef(:,:,3) = robot_arms{3}.frames_(1:3,1:3,index_eef);
% setup bed adapter
...
while (total_data<=10)
    total_data
    index_point_clouds = randi([1 length(point_clouds_prostatectomy_workspace)],1,1);
    p_target_region = point_clouds_prostatectomy_workspace(:,index_point_clouds);
    ...
    for switch_arm = 1 : 2
        ...
        for index_sample = 1 : sample_rate : down_sample
            ...
            if do_plot
                ...
                %                 hold on
                prostatectomy_workspace_transformed = transformSTL(vertex_prostatectomy_macro_workspace,rotation_setup,translation_setup);
                rgba = [1 0 0 0.3];
                PlotStl(prostatectomy_workspace_transformed,rgba);
                hold on
                axis([-1 1 -0.8 0.8 -0.1 2])
...
save('..\export\prostatectomy_macro_collision_data.mat','q_store','joint_percentage','q_store_collision','collision_store')