...
% Run 'ImportLogSponge.m' first to generate the data for this script.
...
for index_sample = 1 : num_pose
    ...
    p_left_cur_in_w = p_camera_cur + rotation_camera_cur * p_left_cur;
    rotation_left_cur_in_w = rotation_camera_cur * rotation_left_cur;
    tool_path_left(:,:,index_sample) = eye(4);
    tool_path_left(1:3,4,index_sample) = rotation_camera_init' * (p_left_cur_in_w - p_camera_init);
    tool_path_left(1:3,1:3,index_sample) = rotation_camera_init' * rotation_left_cur_in_w;

    p_right_cur = arm1(index_sample,1:3)';
    rotation_right_cur = QuaternionToRotation(arm1(index_sample,4:7)');
    p_right_cur_in_w = p_camera_cur + rotation_camera_cur * p_right_cur;
    rotation_right_cur_in_w = rotation_camera_cur * rotation_right_cur;
    tool_path_right(:,:,index_sample) = eye(4);
    tool_path_right(1:3,4,index_sample) = rotation_camera_init' * (p_right_cur_in_w - p_camera_init);
    tool_path_right(1:3,1:3,index_sample) = rotation_camera_init' * rotation_right_cur_in_w;

end
save('export/egg_sponge_user03_tool_path_raw_with_angle.mat','tool_path_left','tool_path_right','tool_path_camera','time_log','gripper_left','gripper_right')
if do_plot
